...
export class AppGanttCustomViewExampleComponent implements OnInit {
    ...
    barClick(event: GanttBarClickEvent) {
        this.thyNotify.info('Event: barClick', `tu as cliqué        [${event.item.title}]`);
    }
    ...
    lineClick(event: GanttLineClickEvent) {
        this.thyNotify.info(
            'Event: lineClick',
            `tu as cliqué
        ${event.source.title} 到 ${event.target.title} 的关联线`
        );
    }
    ...
    selectedChange(event: GanttSelectedEvent) {
        this.thyNotify.info(
            ...
            item de id pour
            ${(event.selectedValue as GanttItem[]).map((item) => item.id).join('、')}`
        );
    }
